...
class TransformerModel:
    ...
    def updater(self, optimizer=None, lr=1e-4, scheduler=None):
        ...
        if self.optimizer is None:
            self.optimizer = AdamW(
                self.transformer.parameters(), lr=lr, correct_bias=False
            )
    ...
    def train(self, train_dataset, valid_dataset, epochs=1, verbosity=4):
        ...
        for epoch in tqdm_notebook(range(epochs)):
            ...
            for i, batch in enumerate(train_dataset.dataloader):
                ...
                if self.scheduler != False:
                    torch.nn.utils.clip_grad_norm_(
                        self.transformer.parameters(), self.clip
                    )
                self.optimizer.step()
                if self.scheduler != False:
                    self.scheduler.step()
                ...
                if i % verbosity == verbosity - 1:
                    print(
                        "Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}".format(
                            epoch + 1, epochs, i + 1, total_step, tr_loss / i
                        )
                    )
            ...
            print(
                "\033[1m"
                + "Epoch [{}/{}], Train_micro_avg: {:.4f}, Valid_micro_avg: {:.4f}".format(
                    epoch + 1, epochs, train_epoch_accuracy, valid_epoch_accuracy
                )
                + "\033[0m"
            )
