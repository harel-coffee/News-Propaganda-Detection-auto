...
class Dataset:
    def __init__(self, articles_folder=None, labels_file=None, df=None):
        assert not (
            (articles_folder == None or labels_file == None) and df == None
        ), "Inputs Invalid"
    ...
    def read(self):
        (
            ...
            span_ends,
            self.gold_labels,
        ) = read_predictions_from_file(self.labels_file)
        self.spans = [
            int(end) - int(start) for start, end in zip(span_starts, span_ends)
        ]
        print(
            "Read %d annotations from %d articles"
            % (len(span_starts), len(set(articles_id)))
        )
        self.sentences = [
            self.articles[id][int(start) : int(end)]
    ...
    def split(self, test_size=0.1, seed=1234):
        a, b = train_test_split(
            self.df, stratify=self.df["Labels"], test_size=test_size, random_state=seed
        )
...
class TransformerDataset:
    def __init__(self, df):
        self.clean()
        self.sentences = df["Sentences"].apply(
            lambda x: "[CLS] {} [SEP]".format(self.clean(x))
        )
        self.le = LE()
    ...
    def tokenize(
        ...
        self.tokenized_texts = [
            self.tokenizer.tokenize(sent) for sent in self.sentences
        ]
        if verbosity:
    ...
    def encode(self, MAX_LEN=90):
        ...
        for i in tqdm_notebook(range(len(self.tokenized_texts))):
            input_ids.append(
                self.tokenizer.convert_tokens_to_ids(self.tokenized_texts[i])
            )
