...
def label_article_tokens(article_id):
    ...
    first_letter_index = (
        []
    )  # creating a list of charecter index of the first charecter of every token
    index = 0
    ...
    binary_labels = [0] * len(
        token_list
    )  # Assigning label of 1 (is propanagada) if first index of token lies in spans
    ...
    for i in range(len(labelled_tokens_list) - 1):
        if (labelled_tokens_list[i][0][0] == '"') & (
            labelled_tokens_list[i + 1][1] == 1
        ):
    ...
    assert len(raw) == len(raw_new), str(
        "Cannot regenerate the raw text from these tokens "
        + str(len(raw))
        + " != "
        + str(len(raw_new))
    )
...
def getLabeledWords(article_id):
    ...
    for i in range(len(wordlist)):
        for span in spans:
            if (wordlist[i][0] in range(span[0], span[1] + 1)) & (
                wordlist[i][1] in range(span[0], span[1] + 1)
            ):
                labels[i] = 1
                break
