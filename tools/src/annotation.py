...
class Annotation(anwol.AnnotationWithOutLabel):
    ...
    def __init__(
        self, label: str = None, start_offset: str = None, end_offset: str = None
    ):
    ...
    def __eq__(self, second_annotation: Annotation):
        ...
        """
        return (
            super().__eq__(second_annotation)
            and self.get_label() == second_annotation.get_label()
        )
    ...
    def get_propaganda_techniques(self) -> list:
        ...
        if self.propaganda_techniques is None:
            logger.error(
                "trying to access propaganda techniques list before initialising the corresponding object"
            )
            sys.exit()
        return self.propaganda_techniques.get_propaganda_techniques_list()

    @classmethod
    def set_propaganda_technique_list_obj(
        cls, propaganda_technique_obj: pt.Propaganda_Techniques
    ) -> None:
        """
    ...
    @staticmethod
    def load_annotation_from_string(
        annotation_string: str, row_num: int = None, filename: str = None
    ) -> (Annotation, str):
        """
        ...
        if len(row) != 4:
            logger.error(
                "Row%s%s is supposed to have 4 columns. Found %d: -%s-."
                % (
                    " " + str(row_num) if row_num is not None else "",
                    " in file " + filename if filename is not None else "",
                    len(row),
                    annotation_string,
                )
            )
        ...
        try:
            ...
            logger.error(
                "The column %d in row%s%s is supposed to be an integer: -%s-"
                % (
                    Annotation.FRAGMENT_START_COL,
                    " " + str(row_num) if row_num is not None else "",
                    " in file " + filename if filename is not None else "",
                    annotation_string,
                )
    ...
    def is_technique_name_valid(self) -> bool:
        ...
        if not self.propaganda_techniques.is_valid_technique(self.get_label()):
            logger.error(
                "label %s is not valid. Possible values are: %s"
                % (self.get_label(), self.propaganda_techniques)
            )
            return False
        return True
