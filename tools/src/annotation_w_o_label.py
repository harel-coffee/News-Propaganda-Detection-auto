...
class AnnotationWithOutLabel(object):
    ...
    def is_span_equal_to(self, second_annotation: AnnotationWithOutLabel) -> bool:
        ...
        """
        if (
            self.get_start_offset() != second_annotation.get_start_offset()
            or self.get_end_offset() != second_annotation.get_end_offset()
        ):
            return False
        return True
    ...
    def load_annotation_from_string(
        ...
        if len(row) != 3:
            logger.error(
                "Row%s%s is supposed to have 3 columns. Found %d: -%s-."
                % (
        ...
        try:
            ...
            logger.error(
                ...
                % (
                    AnnotationWithOutLabel.FRAGMENT_START_COL,
                    " " + str(row_num) if row_num is not None else "",
                    " in file " + filename if filename is not None else "",
                    annotation_string,
                )
        ...
        try:
            ...
            logger.error(
                "The column %d in row%s%s is supposed to be an integer: -%s-"
                % (
                    AnnotationWithOutLabel.FRAGMENT_END_COL,
                    " " + str(row_num) if row_num is not None else "",
                    " in file " + filename if filename is not None else "",
                    annotation_string,
                )
    ...
    def merge_spans(self, second_annotation: AnnotationWithOutLabel) -> None:
        ...
        self.set_start_offset(
            min(self.get_start_offset(), second_annotation.get_start_offset())
        )
        self.set_end_offset(
    ...
    def is_span_valid(self) -> bool:
        ...
        if self.get_start_offset() < 0 or self.get_end_offset() < 0:
            logger.error(
                "Start and end of position of the fragment must be non-negative: %d, %d"
                % (self.get_start_offset(), self.get_end_offset())
            )
            return False
        if self.get_start_offset() >= self.get_end_offset():
            logger.error(
                "End position of the fragment must be greater than the starting one: start=%d, end=%d"
                % (self.get_start_offset(), self.get_end_offset())
            )
            return False
        return True
