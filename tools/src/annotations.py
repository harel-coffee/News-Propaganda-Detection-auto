...
class Annotations(object):
    ...
    def check_annotation_spans_with_category_matching(
        self, merge_overlapping_spans: bool = False
    ):
        """
        ...
        for article_id in self.get_article_id_list():
            ...
            annotation_list = self.get_article_annotations_obj(
                article_id
            ).groupby_technique()
            if merge_overlapping_spans:
                for technique in annotation_list.keys():
                    for i in range(1, len(annotation_list[technique])):
                        annotation_list[technique][i].merge_spans(
                            annotation_list[technique], i - 1
                        )
    ...
    def compare_annotations_identical_article_lists(
        ...
        if len(self.get_article_id_list()) != len(
            second_annotations.get_article_id_list()
        ):
            logger.error(
                "The number of articles in the annotations is different: %d, %d"
                % (
                    len(self.get_article_id_list()),
                    len(second_annotations.get_article_id_list()),
                )
            )
            return False
        diff = set(self.get_article_id_list()).difference(
            set(second_annotations.get_article_id_list())
        )
        if len(diff) > 0:
        ...
        logger.debug(
            "OK: the list of article ids in the two sets of annotations is identical"
        )
        return True
    ...
    def compare_annotations_identical(self, second_annotations: Annotations) -> bool:
        ...
        for article_id in self.get_article_id_list():
            ...
            an2_article_annotations = second_annotations.get_article_annotations_list(
                article_id
            )
            if len(an1_article_annotations) != len(an2_article_annotations):
                logger.error(
                    "The number of annotations for article %s differs: %d vs %d"
                    % (
                        article_id,
                        len(an1_article_annotations),
                        len(an2_article_annotations),
                    )
                )
                return False
            for an1, an2 in zip(an1_article_annotations, an2_article_annotations):
                if not an1.is_span_equal_to(an2):
                    logger.error(
                        "The spans of the annotations of article %s do not match: [%s, %s] vs [%s, %s]"
                        % (
                            article_id,
                            an1.get_start_offset(),
                            an1.get_end_offset(),
                            an2.get_start_offset(),
                            an2.get_end_offset(),
                        )
                    )
                    return False
    ...
    def align_annotations(self, second_annotations: Annotations) -> None:
        ...
        for article_id in second_annotations.get_article_id_list():
            self.get_article_annotations_obj(article_id).align_annotations(
                second_annotations.get_article_annotations_obj(article_id)
            )
    ...
    def compute_TC_score(self, second_annotations: Annotations):
        ...
        gold_labels = [
            x.get_label() for x in second_annotations.get_full_list_of_annotations()
        ]
        submission_labels = [x.get_label() for x in self.get_full_list_of_annotations()]

        precision = precision_score(
            gold_labels, submission_labels, pos_label=None, average="micro"
        )
        recall = recall_score(
            gold_labels, submission_labels, pos_label=None, average="micro"
        )
        ...
        if an.Annotation.propaganda_techniques is not None:
            ...
            f1_per_class = f1_score(
                ...
                submission_labels,
                average=None,
                labels=propaganda_techniques_list,
            )
    ...
    def TC_score_to_string(
        self, second_annotation: Annotations, output_for_script=False
    ):
        ...
        if an.Annotation.propaganda_techniques is None:  # raise an error
            ...
            precision, recall, f1, f1_per_class = self.compute_TC_score(
                second_annotation
            )
            res_for_screen = "\nF1=%f\nPrecision=%f\nRecall=%f\n%s\n" % (
                precision,
                ...
                "\n".join(
                    [
                        "F1_" + pr + "=" + str(f)
                        for pr, f in zip(
                            an.Annotation.propaganda_techniques.get_propaganda_techniques_list(),
                            f1_per_class,
                        )
                    ]
                ),
    ...
    def load_annotation_list_from_file(self, filename):
        ...
        with open(filename, "r") as f:
            for i, line in enumerate(f.readlines(), 1):
                ann, article_id = an.Annotation.load_annotation_from_string(
    ...
    def load_annotation_list_from_folder(self, folder_name, pattern="*.labels"):
        ...
        if not os.path.exists(folder_name):
            logger.error(
                "trying to load annotations from folder %s, which does not exists"
                % (folder_name)
            )
            return False
        if not os.path.isdir(folder_name):
            logger.error(
                "trying to load annotations from folder %s, which does not appear to be a valid folder"
                % (folder_name)
            )
            return False
        ...
        for filename in file_list:
            self.create_article_annotations_object(
                self._guess_article_id_from_file_name(filename)
            )
