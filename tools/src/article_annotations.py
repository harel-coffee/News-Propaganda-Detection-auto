...
class Articles_annotations(object):
    ...
    def __init__(
        self, spans: anwol.AnnotationWithOutLabel = None, article_id: str = None
    ):
    ...
    def _matching_annotations(
        self, second_annotations: Articles_annotations, i: int
    ) -> list(int, int):

        j2 = i
        while j2 < len(second_annotations) and second_annotations[j2].is_span_equal_to(
            second_annotations[i]
        ):
            j1 = i
    ...
    def adapt_annotation_to_new_text(
        self, original_text: str, new_text: str
            ...
            if (
                ...
                if original_text[i] != " ":
                    print(
                        "different",
                        original_text[i],
                        original_text[i].encode(encoding="UTF-8"),
                        new_text[j],
                    )
                    sys.exit()
                i += 1
    ...
    def align_annotations(self, second_annotations: Articles_annotations) -> None:
        ...
        while i < len(self):
            ...
            if (
                j1 == -1
            ):  # no match, can forward i skipping all annotations with same span
                while i + 1 < len(self) and self[i].is_span_equal_to(self[i + 1]):
                    i += 1

    def _get_annotation_offset_excluding_chars(
        self, article_content: str, fragment_index: int, is_starting_fragment: bool
    ):
        ...
        try:
            while (
                article_content[
                    fragment_index + new_offset + (0 if is_starting_fragment else -1)
                ]
                in self.inadmissible_annotation_boundary_chars
            ):
            ...
            print(
                "accessing char %d+%d+%d=%d of string of len %d"
                % (
                    fragment_index,
                    new_offset,
                    (0 if is_starting_fragment else -1),
                    fragment_index + new_offset + (0 if is_starting_fragment else -1),
                    len(article_content),
                )
            )
    ...
    def check_article_annotations_out_of_boundaries(
        self, article_content: str, fix_errors: bool = False
    ) -> list(bool, bool):
        """
        ...
        for ann in self.get_article_annotations():
            if (
                len(article_content) < ann.get_end_offset()
            ):  # making sure the offsets of the annotation are within the length of the article content
                logger.error(
                    "trying to access fragment beyond article length: %s, article %s length: %d"
                    % (ann, self.get_article_id(), len(article_content))
                )
                if fix_errors:
                    if (
                        ann.get_start_offset() > len(article_content) - 1
                    ):  # the annotation is completely beyond the length of the article
    ...
    def check_article_annotations_start_end_chars(
        self, article_content: str, fix_errors: bool = False
    ) -> list(bool, bool):
        """
        ...
        for ann in self.get_article_annotations():
            ...
            new_start_offset = self._get_annotation_offset_excluding_chars(
                article_content, ann.get_start_offset(), True
            )
            new_end_offset = self._get_annotation_offset_excluding_chars(
                article_content, ann.get_end_offset(), False
            )
            if new_start_offset != 0 or new_end_offset != 0:
                logger.error(
                    "annotation %s in article %s starts or ends with inadmissible characters: -%s-"
                    % (
                        ann,
                        self.get_article_id(),
                        article_content[ann.get_start_offset() : ann.get_end_offset()],
                    )
                )
                annotations_correct = False
                if fix_errors:
    ...
    def check_article_annotations_no_duplicated_annotations(
        self, article_content: str, fix_errors: bool = False
    ) -> bool:
        """
        ...
        for i, ann in enumerate(self.get_article_annotations()):
            ...
            while j < len(self.get_article_annotations()):
                if ann.is_span_equal_to(self[j]):
                    logger.error(
                        "article %s found two identical annotations: %s and %s"
                        % (self.get_article_id(), ann, self[j])
                    )
                    if ann.get_label() == self[j].get_label() and fix_errors:
                        self.remove_annotation(self[j])
                        logger.debug(
                            "The two annotations have identical labels (%s, %s), deleted one."
                            % (ann, self[j])
                        )
                        annotations_need_saving = True
                j += 1
    ...
    def check_article_annotations(
        self,
        article_content: str,
        fix_errors: bool = False,
        check_out_of_boundaries=False,
        check_start_end_chars=False,
        check_duplicated_annotations=False,
    ) -> bool:
        """
        ...
        if check_out_of_boundaries:
            (
                annotations_correct,
                annotations_need_saving,
            ) = self.check_article_annotations_out_of_boundaries(
                article_content, fix_errors
            )

        if check_start_end_chars:
            (
                start_end_chars_correct,
                need_saving,
            ) = self.check_article_annotations_start_end_chars(
                article_content, fix_errors
            )
        ...
        if check_duplicated_annotations:
            (
                no_dup_annotations,
                dup_annotations_need_saving,
            ) = self.check_article_annotations_no_duplicated_annotations(
                article_content, fix_errors
            )
            annotations_need_saving = (
                annotations_need_saving or dup_annotations_need_saving
            )
    ...
    def get_markers_from_spans(self):
        ...
        for i, annotation in enumerate(self.spans, 1):
            self.markers.append(
                (annotation.get_start_offset(), annotation.get_label(), i, "start")
            )
            self.markers.append(
                (annotation.get_end_offset(), annotation.get_label(), i, "end")
            )
    ...
    def has_overlapping_spans(
        self, prop_vs_non_propaganda, merge_overlapping_spans=False
    ):
        """
        ...
        for annotation in self.get_article_annotations():
            ...
            if technique not in annotation_list.keys():
                ...
                if merge_overlapping_spans:
                    ...
                    self.merge_article_annotations(
                        annotation_list[technique], len(annotation_list[technique]) - 1
                    )
                else:
                    for matching_annotation in annotation_list[technique]:
                        if annotation.span_overlapping(matching_annotation):
                            logger.error(
                                "In article %s, the span of the annotation %s, [%s,%s] overlap with "
                                "the following one from the same article:%s, [%s,%s]"
                                % (
                                    self.get_article_id(),
                                    annotation.get_label(),
                                    annotation.get_start_offset(),
                                    annotation.get_end_offset(),
                                    matching_annotation.get_label(),
                                    matching_annotation.get_start_offset(),
                                    matching_annotation.get_end_offset(),
                                )
                            )
                            return False
                    annotation_list[technique].append([annotation])
    ...
    def load_article_annotations_from_csv_file(
        self, filename: str, annotation_class=ans.Annotation
    ):
        """
        ...
        with open(filename, "r") as f:
            for i, line in enumerate(f.readlines(), 1):
                ...
                an, article_id = annotation_class.load_annotation_from_string(
                    line.rstrip(), i, filename
                )
            ...
            if self.curr_marker >= len(self.markers):
                ...
                if self.marker_position() <= curr_output_text_index:
                    if self.is_starting_marker():
                        output_text += (
                            self.start_annotation_effect + self.start_annotation_str
                        )
    ...
    def add_sentence_marker(self, line: str, row_counter: int) -> str:
        ...
        if (
            max(line.find("<span"), line.find("</span")) > -1
        ):  # there is an annotation in this row
    ...
    def technique_index_from_annotation_index(self, x: int) -> int:
        ...
        return self.techniques.indexOf(
            self.marker_label(self.annotation_stack_index_to_markers_index(x))
        )

    def start_annotation_marker_function(
        self, annotations_stack: list, marker_index: int, row_counter: int
    ) -> str:

        return '<span id="row%dannotation%d" class="%s">' % (
            row_counter,
            self.marker_annotation(marker_index),
            " ".join(
                [
                    "technique%d" % (self.technique_index_from_annotation_index(x))
                    for x in annotations_stack + [self.marker_annotation(marker_index)]
                ]
            ),
        )
    ...
    def end_annotation_marker_function(
        self, annotations_stack: list, marker_index: int, row_counter: int
    ) -> str:

        if (
            self.marker_annotation() != annotations_stack[-1]
        ):  # we are facing this case: <t1> <t2> </t1> </t2> and we are about to close </t1> (self.marker_annotation()==</t1>, annotations_stack[-1]==</t2>), however, that case above is not allowed in HTML, therefore we are about to transform it to <t1> <t2> </t2></t1><t2> </t2> below
            new_annotations_stack = annotations_stack[
                annotations_stack.index(self.marker_annotation()) :
            ]
            res = "".join(
                ["</span>" for x in new_annotations_stack]
            )  # closing all tags opened that are supposed to continue after </t2>
            new_annotations_stack.remove(
                self.marker_annotation()
            )  # removing </t1> from annotations_stack copy
            technique_index = self.techniques.indexOf(self.marker_label(marker_index))
            res += '<sup id="row%dannotation%d" class="technique%d">%d</sup>' % (
                row_counter,
                self.marker_annotation(),
                technique_index,
                technique_index,
            ...
            for x in new_annotations_stack:
                new_annotations_stack.remove(
                    x
                )  # self.start_annotation_marker_function() assumes x is not in the annotations_stack variable passed as parameter
                res += self.start_annotation_marker_function(
                    new_annotations_stack,
                    self.annotation_stack_index_to_markers_index(x),
                    row_counter,
                )
            return res
        else:  # end of non-overlapping technique
            technique_index = self.techniques.indexOf(self.marker_label(marker_index))
            return '</span><sup id="row%dannotation%d" class="technique%d">%d</sup>' % (
                row_counter,
                self.marker_annotation(),
                technique_index,
                technique_index,
    ...
    def tag_text_with_annotations(self, original_text, print_line_numbers=False):
        ...
        while curr_output_text_index < len(original_text):
            if self.curr_marker >= len(
                self.markers
            ):  # done marking text, need to flush the remaining content of <original_text> into <output_text>
                output_text += original_text[curr_output_text_index:]
                curr_output_text_index = len(original_text)
            else:  # more markers have to be added to the content string
                if (
                    self.marker_position() <= curr_output_text_index
                ):  # it is time to add a marker
                    techniques_index = self.techniques.indexOf(self.marker_label())
                    techniques_found.add(techniques_index)
                    if self.is_starting_marker():
                        output_text += self.start_annotation_marker_function(
                            annotations_stack, self.curr_marker, row_counter
                        )
                        annotations_stack.append(self.marker_annotation())
                    else:
                        output_text += self.end_annotation_marker_function(
                            annotations_stack, self.curr_marker, row_counter
                        )
                        annotations_stack.remove(self.marker_annotation())
                    self.curr_marker += 1
                else:  # flush string content up to the next marker
                    text_to_be_added = original_text[
                        curr_output_text_index : self.marker_position()
                    ]
        ...
        for technique_index in sorted(techniques_found):
            footnotes += "<div>%d: %s</div>\n" % (
                technique_index,
                self.techniques.get_technique(technique_index),
            )
    ...
    def merge_article_annotations(self, annotations_without_overlapping, i):
        ...
        for j in range(0, i):  # len(annotations_without_overlapping)):
            assert i < len(annotations_without_overlapping) or print(
                i, len(annotations_without_overlapping)
            )
            if j != i and annotations_without_overlapping[i].span_overlapping(
                annotations_without_overlapping[j]
            ):
                ...
                annotations_without_overlapping[j].merge_spans(
                    annotations_without_overlapping[i]
                )
    ...
    def annotations_to_string_csv(self):
        ...
        for span in self.spans:
            ...
            if self.output_format_article_spans:
                span_data.append(
                    "%d\t%d" % (span.get_start_offset(), span.get_end_offset())
                )
