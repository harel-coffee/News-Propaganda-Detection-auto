...
__author__ = "blind for submission"
__copyright__ = "Copyright 2019"
__credits__ = [""]
__license__ = "GPL"
__version__ = "0.1"
__maintainer__ = ""
__email__ = "blind for submission"
__status__ = "Beta"
...
def check_data_file_lists(submission_annotations, gold_annotations, task_name="task3"):
    ...
    if len(gold_annotations.keys()) < len(submission_annotations.keys()):
        logger.error(
            "The number of articles in the submission, %d, is greater than the number of articles in the "
            "reference dataset"
            % (len(submission_annotations.keys()), len(gold_annotations.keys()))
        )
        sys.exit()
    ...
    errors = [
        ...
        if article_id not in gold_annotations.keys()
    ]
    if len(errors) > 0:
        logger.error(
            "The following article_ids in the submission file do not have a correspondence in the reference "
            "dataset: %s\n" % (str(errors))
        )
    ...
    logger.debug(
        "OK: all article ids have a correspondence in the list of articles from the reference dataset"
    )
...
def merge_spans(annotations_without_overlapping, i):
    ...
    for j in range(0, len(annotations_without_overlapping)):
        ...
        if (
            j != i
            and len(
                annotations_without_overlapping[i][1].intersection(
                    annotations_without_overlapping[j][1]
                )
            )
            > 0
        ):
            ...
            annotations_without_overlapping[j][1] = annotations_without_overlapping[j][
                1
            ].union(annotations_without_overlapping[i][1])
    ...
    for article_id in annotations.keys():  # for each article
        ...
        for annotation in annotations[article_id]:
            if merge_overlapping_spans:
                ...
                merge_spans(
                    annotations_without_overlapping,
                    len(annotations_without_overlapping) - 1,
                )
                ...
                for span in spans:
                    if len(span.intersection(annotation[1])) > 0:
                        logger.error(
                            "In article %s, the span [%s,%s] overlap with the following one from the same "
                            "article: [%s,%s]"
                            % (
                                article_id,
                                min(annotation[1]),
...
def check_annotation_spans_with_category_matching(
    annotations, merge_overlapping_spans=False
):
    """
...
def check_format_of_annotation_in_file(row, i, techniques_names, filename):
    ...
    if len(row) != 4:
        logger.error(
            "Row %d in file %s is supposed to have 4 TAB-separated columns. Found %d."
    ...
    if row[TASK_3_TECHNIQUE_NAME_COL] not in techniques_names:
        logger.error(
            "On row %d in file %s the technique name, %s, is incorrect. Possible values are: %s"
    ...
    if int(row[TASK_3_FRAGMENT_START_COL]) < 0 or int(row[TASK_3_FRAGMENT_END_COL]) < 0:
        logger.error(
            "On row %d in file %s, start and end of position of the fragment must be non-negative. "
            "Found values %s, %s"
            % (
                i + 1,
                filename,
    ...
    if int(row[TASK_3_FRAGMENT_START_COL]) >= int(row[TASK_3_FRAGMENT_END_COL]):
        logger.error(
            "On row %d in file %s, end position of the fragment must be greater than the starting "
            "one. Found values %s, %s"
            % (
                i + 1,
                filename,
                row[TASK_3_FRAGMENT_START_COL],
...
def check_article_annotations_format(
    submission_article_annotations, article_id, techniques_names
):
    ...
    for i, row in enumerate(submission_article_annotations):
        ...
        if row[0] not in annotations.keys():
            ...
            for span in annotations[row[0]]:
                if (
                    len(set(range(int(span[0]), int(span[1]))).intersection(curr_span))
                    > 0
                ):
                    logger.error(
                        "On row %d in article %s, the span of the annotation %s, [%s,%s] overlap with the "
    ...
    return sum(
        [
            len(
                [
                    example_annotation
                    for example_annotation in x
                    if example_annotation[0] == technique_name
                ]
            )
...
def compute_score_max(
    submission_annotations,
    gold_annotations,
    technique_names,
    prop_vs_non_propaganda=False,
):
    ...
    technique_Spr_max = {
        propaganda_technique: 0 for propaganda_technique in technique_names
    }
    cumulative_Spr_max = 0
    for article_id in submission_annotations.keys():
        gold_data = gold_annotations[article_id]
        logger.debug(
            "Computing contribution to the score of article id %s\nand tuples %s\n%s\n"
        ...
        for j, sd in enumerate(submission_annotations[article_id]):  # submission_data:
            ...
            for i, gd in enumerate(gold_data):
                if prop_vs_non_propaganda or gd[0] == sd[0]:
                    ...
                    s += (
                        "\tmatch %s %s-%s - %s %s-%s: S(p,r)=|intersect(r, p)|/max(|p|,|r|) = %d/max(%d,%d) = %f (cumulative S(p,r)=%f)\n"
                        % (
                            sd[0],
                            min(sd[1]),
                            max(sd[1]),
                            ...
                            max(gd[1]),
                            intersection,
                            sd_annotation_length,
                            gd_annotation_length,
                            Spr,
                            cumulative_Spr_max,
                        )
                    )
    ...
    p, r, f1 = compute_prec_rec_f1(
        cumulative_Spr_max, prec_denominator, cumulative_Spr_max, rec_denominator
    )
    ...
    if not prop_vs_non_propaganda:
        for technique_name in cumulative_Spr_max.keys():
            prec_tech, rec_tech, f1_tech = compute_prec_rec_f1(
                ...
                compute_technique_frequency(
                    submission_annotations.values(), technique_name
                ),
                cumulative_Spr_max[technique_name],
                compute_technique_frequency(gold_annotations.values(), technique_name),
                False,
            )
            logger.info(
                "%s: P=%f R=%f F1=%f" % (technique_name, prec_tech, rec_tech, f1_tech)
            )
...
def compute_score_min(
        ...
        for j, sd in enumerate(submission_annotations[article_id]):  # submission_data:
            ...
            for i, gd in enumerate(gold_data):
                if prop_vs_non_propaganda or gd[0] == sd[0]:
                    ...
                    s += (
                        "\tmatch %s %s-%s - %s %s-%s: S(p,r)=|intersect(r, p)|/min(|p|,|r|) = %d/min(%d,%d) = %f (cumulative S(p,r)=%f)\n"
                        % (
                            sd[0],
                            min(sd[1]),
                            max(sd[1]),
    ...
    p, r, f1 = compute_prec_rec_f1(
        cumulative_Spr_min, prec_denominator, cumulative_Spr_min, rec_denominator
    )
...
def compute_score_pr(
    ...
    for article_id in submission_annotations.keys():
        ...
        for j, sd in enumerate(
            submission_annotations[article_id]
        ):  # submission annotations for article article_id:
            s = ""
            sd_annotation_length = len(sd[1])
            for i, gd in enumerate(gold_data):
                if prop_vs_non_propaganda or gd[0] == sd[0]:
    ...
    if per_article_evaluation:
        logger.info(
            "Per article evaluation F1=%s"
            % (",".join([str(f1_value) for f1_value in f1_articles]))
        )
...
def compute_prec_rec_f1(
    prec_numerator, prec_denominator, rec_numerator, rec_denominator, print_results=True
):
    ...
    for filename in file_list:
        ...
        with open(filename, "r") as f:
            for row_number, line in enumerate(f.readlines()):
                ...
                check_format_of_annotation_in_file(
                    row, row_number, techniques_names, filename
                )
                ...
                annotations[row[TASK_3_ARTICLE_ID_COL]].append(
                    [
                        ...
                        set(
                            range(
                                int(row[TASK_3_FRAGMENT_START_COL]),
...
def main(args):
    ...
    techniques_names = load_technique_names_from_file(
        args.techniques_file
    )  # load technique names
    submission_annotations = load_annotation_list_from_file(
        user_submission_file, techniques_names
    )
    if gold_folder is None:
        # no gold file provided, perform only some checks on the submission files
        logger.info(
            "Checking format of user submitted file %s" % (user_submission_file)
        )
        ...
        logger.info(
            "Scoring user submitted file %s against gold file %s"
            % (user_submission_file, gold_folder)
        )
        ...
        score_pr = compute_score_pr(
            submission_annotations,
            gold_annotations,
            techniques_names,
            prop_vs_non_propaganda,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Scorer for the Propaganda Techniques corpus task FLC (Fragment-level classification). If only -s option is provided, "
        "only some checks on the submitted file is performed."
    )
    parser.add_argument(
        "-s",
        "--submission-file",
        dest="submission",
        required=True,
        help="file with the submission of the team",
    )
    parser.add_argument(
        "-r",
        "--reference-folder",
        dest="gold",
        required=False,
        help="folder with the gold labels.",
    )
    parser.add_argument(
        "-d",
        "--enable-debug-on-standard-output",
        dest="debug_on_std",
        required=False,
        action="store_true",
        help="Print debug info also on standard output.",
    )
    parser.add_argument(
        "-t",
        "--propaganda-techniques-list-file",
        dest="techniques_file",
        required=False,
        default=TECHNIQUE_NAMES_FILE,
        help="file with the list of propaganda techniques",
    )
    parser.add_argument(
        "-l", "--log-file", dest="log_file", required=False, help="Output logger file."
    )
    parser.add_argument(
        "-f",
        "--fragments-only",
    ...
    parser.add_argument(
        ...
        dest="merge_user_annotations",
        required=False,
        action="store_true",
        default=False,
        help="If the option is added, overlapping user annotations are merged",
    )
    parser.add_argument(
        "-e",
        "--per-example-evaluation",
        dest="per_article_evaluation",
        required=False,
        action="store_true",
        default=False,
        help="Prints the value of the evaluation function for each example/article",
    )
