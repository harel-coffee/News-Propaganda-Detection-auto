...
def check_data_file_task2(submission_annotations, gold_annotations):
    ...
    if len(gold_annotations.keys()) != len(submission_annotations.keys()):
        logger.debug(
            "OK: number of articles in the submission for task 2 is the same as the one in the gold file: %d"
            % (len(gold_annotations.keys()))
        )

    # check that all article ids are correct
    gold_file_article_id_set = set(
        [article_id for article_id in gold_annotations.keys()]
    )
    submission_file_article_id_set = set(
        [article_id for article_id in submission_annotations.keys()]
    )
    intersection_file_list = gold_file_article_id_set.intersection(
        submission_file_article_id_set
    )
    if len(intersection_file_list) != len(gold_annotations):
        logger.error(
            "The list of article ids is not identical.\n"
            ...
            % (
                str(
                    submission_file_article_id_set.difference(gold_file_article_id_set)
                ),
                str(
                    gold_file_article_id_set.difference(submission_file_article_id_set)
                ),
            )
        )
        sys.exit()
    ...
    for article_id in submission_annotations.keys():
        ...
        if len(submission_annotations[article_id]) != len(gold_annotations[article_id]):
            logger.error(
                "The number of sentences for article %s in the submission file and the gold file differ: %d - "
                "%d"
                % (
                    article_id,
        ...
        for submission_sentence, gold_sentence in zip(
            sorted(submission_annotations[article_id]),
            ...
            if submission_sentence[0] != gold_sentence[0]:
                logger.error(
                    "On row %d the sentence id of the submission, %s, does not correspond to the one of the "
                    "gold file, %s" % (i, submission_sentence[0], gold_sentence[0])
                )
                sys.exit()
            # checking that the label is valid
            if submission_sentence[1].lower() not in [
                TASK_2_POSITIVE_LABEL,
                TASK_2_NEGATIVE_LABEL,
            ]:
                logger.error(
                    "On row %d the label of the submission, %s, is not valid. Possible values are: "
                    % (
                        i,
                        submission_sentence[1],
                        [TASK_2_POSITIVE_LABEL, TASK_2_NEGATIVE_LABEL],
                    )
                )
...
def compute_score(submission_labels, gold_labels):
    ...
    precision = precision_score(
        gold_labels, submission_labels, pos_label=TASK_2_POSITIVE_LABEL
    )
    recall = recall_score(
        gold_labels, submission_labels, pos_label=TASK_2_POSITIVE_LABEL
    )
...
def load_sentence_labels_from_file(filename):
    ...
    with open(filename, "r") as f:
        for i, line in enumerate(f.readlines()):
            ...
            annotations[row[TASK_2_ARTICLE_ID_COL]].append(
                [row[TASK_2_SENTENCE_ID_COL], row[TASK_2_LABEL_COL]]
            )
...
def main(args):
    ...
    submission_annotations, is_template_submission = load_sentence_labels_from_file(
        user_submission_file
    )
    gold_annotations, is_template_submission = load_sentence_labels_from_file(gold_file)
    logger.info(
        "Checking user submitted file %s against gold file %s"
        % (user_submission_file, gold_file)
    )
    predictions, gold_labels = check_data_file_task2(
        submission_annotations, gold_annotations
    )
    ...
    if not is_template_submission:
        ...
        if per_article_evaluation:
            ...
            for article_id in sorted(gold_annotations.keys()):
                precision, recall, f1 = compute_score(
                    [label for sent_id, label in submission_annotations[article_id]],
        ...
        logger.info(
            "Not scoring the submission because the gold file has ? instead of gold labels"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="gold",
        required=True,
        help="file with the gold labels.",
    )
    parser.add_argument(
        "-d",
