...
def check_format_of_annotation_in_file(row, i, techniques_names, filename):
    ...
    if len(row) != 3:
        logger.error(
            "Row %d in file %s is supposed to have 3 TAB-separated columns. Found %d."
...
def main(args):
    ...
    techniques_names = [
        "propaganda"
    ]  # load_technique_names_from_file(args.techniques_file)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "Scorer for Task SI on the Propaganda Techniques Corpus. "
    )
    parser.add_argument(
        "-s",
        "--submission-file",
    ...
    parser.add_argument(
        "-o",
        "--output-for-script",
        dest="output_for_script",
        required=False,
        action="store_true",
        default=False,
        help="Prints the output in an easy-to-parse way for a script",
    )
